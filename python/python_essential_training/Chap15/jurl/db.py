...
def listrecs():
    ...
    count = db.countrecs()
    message(
        "There are {} records in the database. Add some more!".format(
            count or "no")
    )
...
def list_pagebar(pageno, numpages):
    ...
    if pageno > 0:
        prevlink = '<a href="{}?pageno={}&prevpage=1">&lt;&lt;</a>'.format(
            linkback, pageno
        )
    if pageno < (numpages - 1):
        nextlink = '<a href="{}?pageno={}&nextpage=1">&gt;&gt;</a>'.format(
            linkback, pageno
        )
    ...
    for n in range(0, numpages):
        if n is pageno:
            ...
            pagebar += '<a href="{}?jumppage={}">{}</a>'.format(
                linkback, n, n + 1)
...
def add():
    ...
    sURL = tURL = ""
    if "shortURL" in v:
        sURL = v.getfirst("shortURL")
    else:
        sURL = ""

    if "targetURL" in v:
        tURL = v.getfirst("targetURL")
    else:
        main_page()

    rec = dict(shortURL=cgi.entity_encode(sURL),
...
def update():
    ...
    id = v.getfirst("id")
    rec = dict(id=id, targetURL=cgi.entity_encode(v.getfirst("targetURL")))
    db.update(id, rec)
...
def shorten(s):
    ...
    m = md5(
        s.encode("utf-8")
    )  # md5 because it's short - doesn't need to be secure or reversible
    out = m.digest()
